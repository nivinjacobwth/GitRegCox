...
# Correlation matrix
R<-as.matrix(cor(x))
ones<-as.matrix(rep(1,p))
temp<-ones%*%t(ones)
I<-as.matrix(diag(p))
# psd matrix
P<- I + temp - R

D <- fear.doj(nrow(x), x, y)

G <- fear.gradient(nrow(x), y, x, beta.old)

for(i in 1:itermax){

beta.new <- sapply(c(1:ncol(x)), function(j){
  
 soft.threshold(D[j]*beta.old[j]-G[j], lambda*((P[,j]%*%abs(beta.old))-(P[j,j]*abs(beta.old[j]))))/
            ( D[j] + lambda*P[j,j] )
...
fear.gradient = function(n,y,z,beta){
